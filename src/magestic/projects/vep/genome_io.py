...
# Set VEP_BASE_DIR environment variable to your base directory
# Falls back to repository root if not set
_REPO_ROOT = Path(__file__).parent
BASE_DIR = Path(os.environ.get("VEP_BASE_DIR", _REPO_ROOT))

# Default reference genome paths (relative to BASE_DIR or use VEP_REFERENCE_DIR)
_REF_DIR = Path(os.environ.get("VEP_REFERENCE_DIR", BASE_DIR / "reference_data"))
DEFAULT_S288C_PATH = _REF_DIR / "S288C_reference_sequence_R64-5-1_20240529.chrnames.fsa"
DEFAULT_MAGESTIC_PATH = Path(os.environ.get(
    "MAGESTIC_GENOME_PATH",
    _REF_DIR / "MAGESTIC_background_strain.fasta"
))

# Default strain genomes directory (set VEP_STRAIN_GENOMES_DIR or provide path explicitly)
DEFAULT_STRAIN_GENOMES_DIR = Path(os.environ.get(
    "VEP_STRAIN_GENOMES_DIR",
    BASE_DIR / "strain_genomes"
))
...
def load_strain_chromosome(
    ...
    # 1. Flat: chromosomes/{strain}_{chrom}.fasta (current build output)
    # 2. Nested: chromosomes/{strain}/{chrom}.fasta (original convention)
    chrom_file_flat = strain_genomes_dir / "chromosomes" / f"{strain}_{chrom}.fasta"
    chrom_file_nested = strain_genomes_dir / "chromosomes" / strain / f"{chrom}.fasta"

    if chrom_file_flat.exists():
        chrom_file = chrom_file_flat
    elif chrom_file_nested.exists():
        chrom_file = chrom_file_nested
    else:
        raise FileNotFoundError(
            f"Strain chromosome file not found. Tried:\n"
            f"  {chrom_file_flat}\n"
            f"  {chrom_file_nested}\n"
