...
def detect_linked_variants(
    ...
    **Linking Rules (Updated 2026-02-23):**
    ...
    def get_group_span(group: List[PairwiseVariant]) -> int:
        """Calculate span of a group (max pos - min pos)."""
        if not group:
            return 0
        positions = [v.s288c_pos for v in group]
    ...
    for i in range(1, len(sorted_vars)):
        ...
        if current_var.s288c_pos - prev_var.s288c_pos <= link_dist:
            # Also check if adding this would exceed max_distance (50bp span)
            test_group = current_group + [current_var]
            if get_group_span(test_group) <= max_distance:
                current_group.append(current_var)
            else:
                # Would exceed max span - close current group and start new
