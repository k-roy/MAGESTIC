...
def load_transcript_coverage(bedgraph_filename: str) -> dict:
    """
    Load per-position transcript coverage from a bedgraph file.

    The bedgraph format is: ``chrom  start  end  reads`` (0-based, half-open).
    Only nuclear chromosomes are loaded (mitochondrial entries are skipped).

    Parameters
    ----------
    bedgraph_filename : str
        Path to a strand-specific bedgraph file (e.g. from TIF-seq or RNA-seq).

    Returns
    -------
    dict
        Nested dict ``{chrom: {coord: read_count}}`` for all positions with
        non-zero coverage. Chromosomes with no coverage are absent.

    Notes
    -----
    Used to orient donor DNA so the single-stranded retron donor is
    sense relative to the predominant transcript at each locus.
    """
    chrom_to_coord_to_reads: dict = {}
    with open(bedgraph_filename) as infile:
        for line in infile:
            chrom, start, end, reads = line.strip().split()
            if "M" not in chrom:
                start_i, end_i, reads_i = int(start), int(end), int(reads)
                chrom_to_coord_to_reads.setdefault(chrom, {})
                for coord in range(start_i, end_i):
                    chrom_to_coord_to_reads[chrom][coord] = reads_i
    logger.info("Loaded transcript coverage from %s", bedgraph_filename)
    return chrom_to_coord_to_reads
