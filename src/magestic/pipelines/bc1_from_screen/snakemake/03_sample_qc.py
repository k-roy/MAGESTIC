#!/usr/bin/env python3
"""
Step 02b: Sample Quality Control for BC1 Screen Data

Run AFTER Step 02 (bc1 counting) and BEFORE Step 03 (error detection/tier classification).
...
Usage:
    python 02b_sample_qc.py \\
        --input-long-table bc1_counts_long_format.tsv \\
        --keyfile combined_key.tsv \\
        --output-dir qc/ \\
        --plots-dir qc_plots/

Output:
    {output_dir}/sample_qc_summary.tsv  - All metrics per sample
    {output_dir}/flagged_samples.tsv    - Samples failing QC
    {output_dir}/pca_coordinates.tsv    - PCA coordinates
    {plots_dir}/*.png                   - QC visualizations
...
def compute_sample_metrics_chunked(
    ...
    for chunk_num, chunk in enumerate(pd.read_csv(
        ...
        # Vectorized aggregation per (sample, bc1) - much faster than iterrows
        agg = chunk.groupby(["sample_name", "bc1"])["sample_counts"].sum()
        for (sample, bc1), count in agg.items():
...
def compute_replicate_concordance(
    ...
    for chunk in pd.read_csv(long_table_path, sep="\t", chunksize=config.chunksize):
        ...
        agg = chunk_filtered.groupby(["sample_name", "bc1"])["sample_counts"].sum()
        for (sample, bc1), count in agg.items():
            sample_bc1_counts[sample][bc1] += count
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Step 02b: Sample Quality Control for BC1 Screen Data",
...
Examples:
    ...
    python 02b_sample_qc.py \\
        ...
        --output-dir qc/ \\
        --min-reads 100000 \\
        --min-correlation 0.90
"""
    )
    parser.add_argument("--input-long-table", type=Path, required=True,
                        help="Path to BC1 counts long format TSV")
    parser.add_argument("--keyfile", type=Path, required=True,
                        help="Path to combined sample keyfile TSV")
    parser.add_argument("--output-dir", type=Path, required=True,
                        help="Output directory for QC summary files")
    parser.add_argument("--plots-dir", type=Path, default=None,
                        help="Output directory for QC plots (default: output-dir/plots)")

    # Optional threshold parameters
    parser.add_argument("--min-reads", type=int, default=50000,
                        help="Minimum total reads threshold (default: 50000)")
    parser.add_argument("--min-correlation", type=float, default=0.85,
                        help="Minimum replicate correlation (default: 0.85)")
    parser.add_argument("--min-entropy", type=float, default=0.5,
                        help="Minimum normalized entropy (default: 0.5)")
    parser.add_argument("--pca-outlier-threshold", type=float, default=3.0,
                        help="PCA outlier z-score threshold (default: 3.0)")
    ...
    print(f"=" * 70)
    print(f"Step 02b: Sample Quality Control")
    ...
    config = SampleQCConfig(
        ...
        min_replicate_correlation=args.min_correlation,
        min_normalized_entropy=args.min_entropy,
        pca_outlier_threshold=args.pca_outlier_threshold,
    )
    ...
    # Check inputs
    if not args.input_long_table.exists():
        print(f"ERROR: Long table not found: {args.input_long_table}")
        sys.exit(1)
    ...
    # Step 1: Load keyfile
    # ========================================================================
    print(f"\n[1/6] Loading keyfile: {args.keyfile}")
    ...
    sample_metrics = compute_sample_metrics_chunked(args.input_long_table, keyfile, config)
    ...
    concordance_df = compute_replicate_concordance(
        sample_metrics, args.input_long_table, keyfile, config
    )
    ...
    pca_df, distance_df = compute_pca_and_outliers(sample_metrics, args.input_long_table, config)
    ...
    plot_pca(pca_df, sample_metrics, list(flagged_samples), plots_dir)
    plot_read_count_distribution(sample_metrics, config, plots_dir)
    plot_clustering_dendrogram(pca_df, sample_metrics, plots_dir)
    plot_correlation_heatmap(sample_metrics, plots_dir)
    ...
    # Sample QC summary
    summary_path = args.output_dir / "sample_qc_summary.tsv"
    sample_metrics.to_csv(summary_path, sep="\t", index=False)
    print(f"  Saved: {summary_path}")

    # Flagged samples
    flagged_path = args.output_dir / "flagged_samples.tsv"
    flagged_df.to_csv(flagged_path, sep="\t", index=False)
    print(f"  Saved: {flagged_path}")

    # PCA coordinates
    pca_path = args.output_dir / "pca_coordinates.tsv"
