...
cd $FASTQ_DIR
for sample in 20250815_plate_5B_*_R1.fastq.gz;
do prefix=${sample%_R1.fastq.gz};
echo $prefix; 
bbduk.sh in1=$FASTQ_DIR$prefix"_R1.fastq.gz" in2=$FASTQ_DIR$prefix"_R2.fastq.gz" out1=$TRIMMED_READS_DIR$prefix"_trimmed_R1.fastq.gz" out2=$TRIMMED_READS_DIR$prefix"_trimmed_R2.fastq.gz" literal=AGATCGGAAGAGCGTCGTGTAGGGAAAGA overwrite=false ftl=20 ktrim=r k=23 mink=11 hdist=1 tpe tbo; 
done